...
class AbridgedResponse extends Writable implements CORSAnywhereServerResponse {
  ...
  setHeader(name: string, value: string | number | string[]): AbridgedResponse {
    if (this._headersSent) {
      throw new Error("Headers already sent");
    }
...
export class BitmapContentSender extends BitmapEmbedder {
  ...
  public send(response: ServerResponse, overwriteHeader?: OutgoingHttpHeaders) {
    this._image();
    ...
    Readable.from([this.bitmap!.getLittleEndian()]).pipe(gzip).pipe(response);
