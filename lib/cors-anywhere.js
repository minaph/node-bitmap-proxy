...
// var Bitmap = require("../dist/bitmap").Bitmap
      ...
      pipe: function(proxyReq) {
        ...
        proxyReq.on = function(eventName, listener) {
          ...
          return proxyReqOn.call(this, 'response', function(proxyRes) {　
            // ここのProxyResをすり替える
            // すり替えた先のresがendになったら、暗号などの処理をして、それで本来のresに送る。
            // onProxyResponseが真を返すことを確定させたタイミングですり替える（つまり、onProxyResponseには替え玉だけ送り込んでおけばいいってこと。）。
            // その後のCORSヘッダ処理を遅延させておいて、
            // すり替えたres（実）の方にデータを貯める。実にデータが溜まったら、
            // 暗号化して皮の方にデータを送り、CORS処理を進める。
            if (onProxyResponse(proxy, proxyReq, proxyRes, req, res)) {
              try {
                if(req?.corsAnywhere?.handleInitialRequest) {
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  // var bitmap = Bitmap.fromString(proxyRes.)

  // req.listener_of_bitamp(proxyRes, res => withCORS(res.headers, req))
